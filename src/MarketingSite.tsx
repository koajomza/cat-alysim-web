...
type DownloadInfo={installerUrl:string;installerVersion:string};
...
function accountLicenseLabel(ctx:AuthContext|null){const p=(ctx?.profile||ctx?.license?.profile||{}) as Record<string,unknown>;const type=String(ctx?.license?.license_type||p.status||'trial');return type==='paid'||type==='admin'||p.status==='active'||p.paid_at?'Full License':'Trial'}
function useLatestDownload(){
 const [download,setDownload]=useState<DownloadInfo>({installerUrl:release.installerUrl,installerVersion:release.installerVersion});
 useEffect(()=>{let alive=true;fetch(`${release.latestJson}?v=${Date.now()}`,{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(data=>{
  const url=data?.full_installer?.url||data?.installer_url||data?.download_url;
  const version=data?.bootstrap_installer_version||data?.installer_version||data?.version;
  if(alive&&url)setDownload({installerUrl:url,installerVersion:version||release.installerVersion});
...
function Header({auth,name,userMenu,setUserMenu,menu,setMenu,signOut,download}:{auth:AuthContext|null;name:string;userMenu:boolean;setUserMenu:(v:boolean|((v:boolean)=>boolean))=>void;menu:boolean;setMenu:(v:boolean|((v:boolean)=>boolean))=>void;signOut:()=>void;download:DownloadInfo}){
 const license=accountLicenseLabel(auth);
 const authActions=auth?<div className="user-nav"><button className="user-nav-button polished" onClick={()=>setUserMenu(v=>!v)}><span className="user-mini-avatar">{name.slice(0,1).toUpperCase()}</span><span className="user-mini-copy"><b>{name}</b><small>{license}</small></span><ChevronDown size={15}/></button>{userMenu&&<div className="user-dropdown"><a href="/trial/drugs/"><FileText size={16}/>เข้า Trial Drugs</a><a href="/payment/"><CreditCard size={16}/>{license==='Full License'?'ดูสถานะชำระเงิน':'อัปเกรดตลอดชีพ 5,000'}</a><a href={download.installerUrl}><Download size={16}/>ดาวน์โหลดโปรแกรม</a><button onClick={signOut}><LogOut size={16}/>ออกจากระบบ</button></div>}</div>:<><a className="nav-login" href="/trial/drugs/?auth=login"><LogIn size={16}/>เข้าสู่ระบบ</a><a className="nav-trial" href="/trial/drugs/?auth=signup"><UserPlus size={16}/>สมัครสมาชิก</a></>;
...
function DownloadBand({download}:{download:DownloadInfo}){return <section id="download" className="download-band release-band"><div><span>DOWNLOAD</span><h2>ดาวน์โหลดโปรแกรม Windows</h2><p>ติดตั้งด้วยไฟล์ .exe แล้วเข้าสู่ระบบด้วยบัญชี CAT-ALYSIM ของคุณ</p><small>เวอร์ชัน {download.installerVersion}</small></div><div><a className="btn-light download-exe" href={download.installerUrl}>ดาวน์โหลด .exe <Download size={18}/></a></div></section>}
...
export default function MarketingSite({page='home'}:{page?:PageMode}){
 ...
 return <div className="marketing-site"><Header auth={auth} name={name} userMenu={userMenu} setUserMenu={setUserMenu} menu={menu} setMenu={setMenu} signOut={signOut} download={download}/>{page==='program'?<ProgramPageContent/>:page==='features'?<FeaturesPageContent/>:page==='photoEvidence'?<PhotoEvidencePageContent/>:page==='crimeScene'?<CrimeScenePageContent/>:page==='trafficFine'?<TrafficFinePageContent/>:<HomePage download={download}/>}<Footer download={download}/></div>
}
